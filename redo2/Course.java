...
public class Course { // made -- test left
    ...
    public static Course create_course(Professor assigned_prof) {
        ...
        if (course_db.containsKey(course.course_id)) {
            ...
            // Add course to semester-specific course list
            semester_course_db.computeIfAbsent(course.offered_semester, k -> new ArrayList<>()).add(course);
    ...
    public static void update_course(String course_id) {
        if (course_db.containsKey(course_id)) {
            ...
            while (true) {
                ...
                System.out.println("4. Update Syllabus");
                System.out.println("5. Update Timings");
                System.out.println("6. Update Credits");
                System.out.println("7. Update Semester");
                System.out.println("8. Update Enrollment Limit");
                System.out.println("9. Manage Prerequisites");
                System.out.print("Enter your choice: ");
                Integer choice = sc.nextInt();
                ...
                switch (choice) {
                    ...
                    case 2:
                        ...
                        course.set_course_id(new_id);
                        course_db.remove(prev_id);
                        course_db.put(new_id, course);
                        Student.changes_in_completed_courses(prev_id, new_id);
                        break;
                    case 3:
                        System.out.print("Enter new course description: ");
                        course.set_course_description(sc.nextLine());
                    ...
                    case 7:
                        System.out.print("Enter new offered semester: ");
                        Integer prev_semester = course.get_offered_semester();
                        course.set_offered_semester(sc.nextInt());
                        sc.nextLine();  // Consume newline
                        semester_course_db.get(prev_semester).remove(course);
                        semester_course_db.computeIfAbsent(course.get_offered_semester(), k -> new ArrayList<>()).add(course);
                        break;
                    case 8:
                        System.out.print("Enter new enrollment limit: ");
                        course.set_enrollment_limit(sc.nextInt());
    ...
    private static void manage_prerequisites(Course course) { // added for update course
        course.show_prerequisites();
        ...
        while (true) {
            ...
            System.out.println("2. Remove prerequisites");
            System.out.print("Enter your choice: ");
            Integer choice2 = sc.nextInt();
            sc.nextLine();  // Consume newline
            ...
            switch (choice2) {
                case 1:
                    ...
                    String prereq_id = sc.nextLine();
                    if (course_db.containsKey(prereq_id)) {
                        Course prereq = course_db.get(prereq_id);
                        course.add_prerequisite(prereq);
                        prereq.add_prerequisite_of(course);
                    } else {
                        System.out.println("Prerequisite course not found.");
                    }
                    break;
                case 2:
                    System.out.print("Enter the course ID of the prerequisite to remove: ");
                    String remove_prereq_id = sc.nextLine();
                    if (course_db.containsKey(remove_prereq_id)) {
                        Course prereq = course_db.get(remove_prereq_id);
                        course.remove_prerequisite(prereq);
                        prereq.remove_prerequisite_of(course);
                    } else {
                        System.out.println("Course not found.");
                    }
                    break;
    ...
    public void enroll_student(Student student) {
        if (this.enrolled_students.containsKey(student.get_student_roll_number())) {
            ...
            for (Course prerequisite : this.course_prerequisites) {
                if (!student.completed_courses.containsKey(prerequisite.get_course_id())) {
                    System.out.println("Prerequisite not met: " + prerequisite.get_course_name());
            ...
            if (prerequisitesMet) {
                ...
                System.out.println("Cannot enroll student due to missing prerequisites.");
            }
        }
    } // modified
