...
public class Student extends ProfStudComman{ // Left
    ...
    public void update_details() {
        ...
        System.out.println("4. Current Semester");
        System.out.println("5. Grades");
        System.out.println("6. SGPAs");
        ...
        Student student = student_db.get(this.student_roll_number);
        switch (update_choice) {
            ...
            case 6:
                System.out.print("Enter the Semester for which you want to update the SGPA: ");
                Integer sem = sc.nextInt();
                System.out.print("Enter the new SGPA: ");
                Float new_sgpa = sc.nextFloat();
                student.set_sgpa(sem, new_sgpa);
                student.calculate_cgpa();
                break;
        ...
        if (this.credits_registered < credit_limit) {
            ...
            if (Course.course_db.containsKey(course_id)) {
                ...
                if (course.get_offered_semester() == this.current_semester) {
                    ...
                    for (String prerequisite : course.get_prerequisites()) {
                        if (!this.completed_courses.containsKey(prerequisite)) {
                            prerequisitesMet = false;
                            System.out.println("You haven't completed the prerequisite: " + prerequisite);
                        }
                    }
                    if (prerequisitesMet) {
                        course.enroll_student(this);
